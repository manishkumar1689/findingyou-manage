...
import { GrahaObj } from "./interfaces";
import { julToDateParts, julToUnixTime } from "./julian-date";
...
export const degAsDms = (
  ...
  if (precision >= 0) {
    ...
    const secDisplay = parseFloat(sec3dec);
    if (secDisplay >= 60 && secDisplay < 60.1) {
      dms.min += 1;
      sec3dec = "0";
    }
    strSecs = ` ${zeroPad(sec3dec, 2)}"`;
  }
...
export const toDateTime = (strDate: string, applyOffset = true) => {
...
export const toGrahaObj = (ref: any): GrahaObj => {
  let graha = { num: -1, jyNum: -1, key: "", name: "", icon: "" };
  let gr = null;
  if (isNumeric(ref)) {
    const num = parseInt(ref);
    gr = grahaValues.find((row) => row.num === num);
  } else if (notEmptyString(ref, 1)) {
    gr = grahaValues.find((row) => row.key === ref);
  } else if (ref instanceof Object) {
    gr = ref;
  }
  if (!gr) {
    gr = refValues.find((row) => row.key === ref);
  }
  if (gr) {
    const { num, jyNum, key, name, icon } = gr;
    graha = { num, jyNum, key, name, icon };
  }
  return graha;
};
...
export const yesNo = (ref: any): string => {
  let str = "";
  switch (typeof ref) {
    case "boolean":
      str = ref ? "yes" : "no";
...
/* export const testJulConversion = () => {
  const jd = 2440490;
  const jsTzOffset = getLocaleOffsetSecs();
  const dt = julToISODate(jd, 0, true);
  const [hours, mins, secs] = dt
    .split("T")
    .pop()
    .split(":")
    .map(smartCastInt);
  const decHours = hours + mins / 60;
  const diff = 12 - decHours;
  const diffSecs = diff * 3600;
  const tzOffset = diffSecs;
  window.tzOffset = tzOffset;
  window.jsTzOffset = jsTzOffset;
  Vue.ls.set("timezone", {
    mins: diff * 60,
    tzOffset,
    jsTzOffset,
...
/* export const julToDateFormatValid = (
  ...
  const dtS = julToISODate(jd, tzOffset, true);
...
export const shortenName = (name = null) => {
  let str = "";
  if (notEmptyString(name)) {
    const parts = name.split(" ");
    const numParts = parts.length;
    const excludeSuffixes = ["jr", "jr."];
    if (numParts > 1) {
      let lastPart = parts[numParts - 1];
      if (numParts > 2 && excludeSuffixes.includes(lastPart.toLowerCase())) {
        lastPart = parts[numParts - 2];
      }
      str = [parts[0], lastPart].join(" ");
    } else {
      str = parts[0];
    }
  }
