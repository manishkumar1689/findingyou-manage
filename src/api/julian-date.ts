...
export const julToUnixTime = (jd: number, tzOffset = 0) => {
...
*/
export const julToUnixMillisecs = (jd: number, tzOffset = 0) => {
...
*/
export const unixTimeToJul = (ts = 0) => {
...
export const localTimeZoneOffset = () => {
...
export const dateStringToJulianDay = (dateStr = ""): number => {
  ...
  const tsOffset = dtObj.getTimezoneOffset() * 60;
...
export class JulDate {
  ...
  */
  ...
  get extended() {
    return [this.shortWeekDayName, [this.day, this.shortMonthName,this.year].join(" ")].join(", ");
  }
...
export const julianDayGeoOffsetToStart = (jd = 0, lng = 0, noon = false): number => {
